...
import { consuptions, IStromertrag, IVerbrauchWH, totalConsuptions } from '../../shared/models/consuptions';
import { bezugProMonat, nutzungsAufteilung, überProduktionProMonat } from '../../shared/models/nutzungsaufteilung';
...
export class FormserviceService {
  ...
  private _aufteilung: nutzungsAufteilung | null;
  ...
  private _stromverbrauchWasserMonat: IVerbrauchWH[] = []; 
  private _stromverbrauchHeizenMonat: IVerbrauchWH[]= [];
  ...
  constructor(){
   ...
   this.supabaseService.getConsuptions().then((value)=>{
    this._consuptions = value;
   });
  }
  ...
  private async caculateConsuptions(): Promise<totalConsuptions[]>{
    ...
    if(this._consuptions != null){
      for (const value  of this._consuptions){
        ...
        this._stromverbrauchHeizenMonat.push({monat: value.Monat, consuptions: verbrauchWärme12});
        this._stromverbrauchWasserMonat.push({monat: value.Monat, consuptions: verbrauchWasser}); 
  ...
  private async StromertragProMonat():Promise< IStromertrag[]>{
    //Liste von verbräuchen pro monat
  ...
  private async überProduktion(zeitraum: string,):Promise< überProduktionProMonat[]>{
    ...
    if(resp && aufteilung && this._stromertrag != undefined){
      for (let [index, ertrag] of this._stromertrag.entries()){
          let verbrauchStrom = resp[index].Strom *  (this.Stromverbrauch()/12); 
          let verbrauchEfahrzeug = resp[index].EFahrzeug * (this.stromverbrauchEfahrzeug()/12);
          let stromverbrauchHeizen = this._stromverbrauchHeizenMonat[index].consuptions;
          let stromverbrauchWasser = this._stromverbrauchWasserMonat[index].consuptions; 
          console.log("Stromertrag not undefined");
          if(zeitraum == '18-6'){
            ertrag.ertrag = 0; 
          }
  ...
  public async amortisationZeit(investitionskosten: number){
    const wartungÖl = 200;
    const wartungPV = 25; 
    const wartungPumpe = 300; 
    let Zinskosten=  this.zinskosten(investitionskosten);
    let k_erspanis: number = await this.GuV() + wartungÖl; 
    let k_em: number = wartungPV +wartungPumpe + Zinskosten; 
    let k_a = k_erspanis - k_em; 
    return investitionskosten / k_a
  }

  private  zinskosten(investitionskosten: number): number{
    if(this._finance.controls['externalFincance'].value){
      let rate:string = this._finance.controls['zinssatz'].value
      let kumzinsz = this.kumzinsz(Number(rate.replace(',', '.'))/12, Number(this._finance.controls['laufzeit'].value.replace(',','.'))*12, investitionskosten, 1,Number(this._finance.controls['laufzeit'].value.replace(',','.'))*12,0); 
  ...
  public async autarkieGrad():Promise< [number, number]>{
    ...
    return [(stromertragJährlich - einspeißungJärhlich)/(stromertragJährlich - einspeißungJärhlich+ Math.abs(bezugJährlich)), this.eingenVerbrauchsanteil(stromertragJährlich, einspeißungJärhlich)]; 
    
  }

  public eingenVerbrauchsanteil(stromertragJährlich:number, einspeißungJärhlich:number):  number{
